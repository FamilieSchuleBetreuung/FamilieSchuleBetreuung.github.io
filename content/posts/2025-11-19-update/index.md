---
title: "Projekt Update: November 18, 2025"
date: 2025-11-18T23:16:52+01:00  
draft: false
categories:
  - ProjektUpdate
tags:
  - projekt_management
  - child_care
---

* WhatsApp Community gegründet, aktuell 285 Mitglieder
* Projekt Streams aufgesetzt:
    * Stream 1: Taktik Gemeindeversammlung
    * Stream 2: Child Care Gemeindeversammlung
    * Stream 3: Zukunftsvision Schule & Betreuung
    * Stream 4: Mobilisierung
    * Stream X: Community Management

# Feedback aus den Streams

## Stream 1: Taktik Gemeindeversammlung
* Datenanalysen und Argumente werden zusammengetragen.
* Primär wird jedoch der letzte offene Gegenvorschlag (GLP) abgewartet.
* Erster Poll bezüglich akzeptabler Preisgestaltung wurde durchgeführt:
  <!-- {{< figure src="poll_maximalbetrag.jpg" alt="Umfrage zur akzeptablen Preisgestaltung für die Gemeindeversammlung" caption="Poll: Akzeptabler Maximalbetrag pro Jahr" width="250">}} -->

  ![]({{< file "poll_maximalbetrag_small.jpg" >}})


## Stream 2: Child Care Gemeindeversammlung
* Durch eine WhatsApp-Umfrage wurde die "allgemeine Nachfrage nach Kita/Hort" während der Gemeindeversammlung abgeklärt:
  * 41x "Ja, Hort"
  *  7x "Ja, Kita".
  * 14x "nein"
* Dies bestätigt eine klare Nachfrage nach Kinderbetreuung an der Gemeindeversammlung.
* Es wurden zwei Räumlichkeiten an zentraler Lage angeschaut (Idee: 1x Hort, 1x Kita).
* Detailabklärungen laufen aktuell in diverse Richtungen (Was dürfen wir, was müssen wir, was brauchen wir etc.).
    ![]({{< file "poll_kinderbetreuung_gv_small.jpg" >}})


## Stream 3: Zukunftsvision Schule & Betreuung
* Eine erste Umfrage zum Interesse an einer Tagesschule wurde durchgeführt: 57x ja, 3x eher ja, 10x kommt sehr auf die Details an, 1x eher nein, 2x nein.
    ![]({{< file "poll_tagesschule_small.jpg" >}})
    

## Stream 4: Mobilisierung
* Keine Aktivitäten, noch sehr abhängig von gewählter Taktik.


## Stream X: Community Management
* Zur Sicherheit wurde ein zweiter Administrator benannt (Max Hacker), bis sich ein kleines Team konstituiert hat, um die Sache richtig zu übernehmen.
